module.exports = [{
    rid: 'goramen',
    contents: [{
        number: 1,
        userImg: '../img/sc-img/members/Jellyfish.jpg',
        userName: '알라코',
        likeCount: 0,
        trashCount: 0,
        achievement: [{
            emblem: '../img/sc-img/achievement/noodle.svg',
            achieve: '면요리 100회 리뷰'
        }, {
            emblem: '../img/sc-img/achievement/bbq.svg',
            achieve: '고기요리 100회 리뷰'
        }, {
            emblem: '../img/sc-img/achievement/pizza.svg',
            achieve: '피자 100회 리뷰'
        }],
        foodImg: '../img/sc-img/restaurants/goramen_01.jpg',
        eaten: [{
            menu: '고라멘 1 그릇 8000 원'
        }, {
            menu: '챠슈추가 1000 원'
        }],
        detail: '돼지뼈, 닭뼈, 생선 육수국물이 <br>' +
        '아주 진해서 밥말아 먹기 딱 좋은 라멘!!',
        tag: [{
            item: "고라멘"
        }, {
            item: "돈코츠라멘"
        }, {
            item: "라멘"
        }, {
            item: "챠슈"
        },{
            item: "고라멘"
        }, {
            item: "돈코츠라멘"
        }, {
            item: "라멘"
        }, {
            item: "챠슈"
        },{
            item: "고라멘"
        }, {
            item: "돈코츠라멘"
        }, {
            item: "라멘"
        }, {
            item: "챠슈"
        }]
    }, {
        number: 2,
        userImg: '../img/sc-img/members/Koala.jpg',
        userName: '알라코',
        likeCount: 1,
        trashCount: 2,
        achievement: [{
            emblem: '../img/sc-img/achievement/noodle.svg'
        }, {
            emblem: '../img/sc-img/achievement/bbq.svg'
        }, {
            emblem: '../img/sc-img/achievement/pizza.svg'
        }],
        foodImg: '../img/sc-img/restaurants/goramen_02.jpg',
        eaten: [{
            menu: '고라멘 1 그릇 7000 원'
        }, {
            menu: '챠슈추가 1000 원'
        }],
        detail: '친구의 단골집인 일본라멘집.<br>' +
        '처음 먹어본 일본 라멘집이라 그냥 맛 있는 집이라고 생각 했었는데, ' +
        '다른 일본 라면집을 먹어보니 확실히 맛집이란걸 알앗다.<br>' +
        '다른 라멘집은 인스턴트 라면 같은 일본 라면 집이라면 여기는 ' +
        '육수를 끓여 만든 라면 국물을 사용하기 때문에 다른데 보다 속이 편안 하고 먹기 좋다.<br>' +
        '그리고 육수농도(보통,진하게), 면(꼬들꼬들, 보통, 퍼진면) 염도(싱겁게, 보통, 짜게) ' +
        '를 커스텀마이징, 정할수 있어서 자기 취향에 맞는 라면을 먹을수 있다.<br>' +
        '밥, 김치는 무료로 제공해준다.(남길경우 5,000원)<br>',
        tag: [{
            item: "고라멘"
        }, {
            item: "돈코츠라멘"
        }, {
            item: "라멘"
        }, {
            item: "챠슈"
        },{
            item: "고라멘"
        }, {
            item: "돈코츠라멘"
        }, {
            item: "라멘"
        }, {
            item: "챠슈"
        },{
            item: "고라멘"
        }, {
            item: "돈코츠라멘"
        }, {
            item: "라멘"
        }, {
            item: "챠슈"
        }]
    }, {
        number: 3,
        userImg: '../img/sc-img/members/Penguins.jpg',
        userName: '펭귄맨',
        likeCount: 110,
        trashCount: 11,
        achievement: [{
            emblem: '../img/sc-img/achievement/noodle.svg'
        }, {
            emblem: '../img/sc-img/achievement/bbq.svg'
        }, {
            emblem: '../img/sc-img/achievement/pizza.svg'
        }],
        foodImg: '../img/sc-img/restaurants/goramen_03.jpg',
        eaten: [{
            menu: '소유라멘 1 그릇 7000 원'
        }],
        detail: '토/일요일에만 판매한다고 하는<br>' +
        '쇼유라멘 먹고왔어요 ^^',
        tag: [{
            item: "돈코츠라멘"
        }, {
            item: "쇼유라멘"
        }, {
            item: "부타동"
        }, {
            item: "고라멘"
        }]
    }, {
        number: 4,
        userImg: '../img/sc-img/members/Penguins.jpg',
        userName: '펭귄맨',
        likeCount: 333,
        trashCount: 222,
        achievement: [{
            emblem: '../img/sc-img/achievement/noodle.svg'
        }, {
            emblem: '../img/sc-img/achievement/bbq.svg'
        }, {
            emblem: '../img/sc-img/achievement/pizza.svg'
        }],
        foodImg: '../img/sc-img/restaurants/goramen_04.jpg',
        eaten: [{
            menu: '카라이 고라멘 1 그릇 9000 원'
        }, {
            menu: '계란추가 1000 원'
        }],
        detail: '가격이 올랐네요... 그래도 너무 맛있어요..',
        tag: [{
            item: "돈코츠라멘"
        }, {
            item: "카라이고라멘"
        }, {
            item: "부타동"
        }, {
            item: "고라멘"
        }]
    }, {
        number: 5,
        userImg: '../img/sc-img/members/Penguins.jpg',
        userName: '펭귄맨',
        likeCount: 11,
        trashCount: 11,
        achievement: [{
            emblem: '../img/sc-img/achievement/noodle.svg'
        }, {
            emblem: '../img/sc-img/achievement/bbq.svg'
        }, {
            emblem: '../img/sc-img/achievement/pizza.svg'
        }],
        foodImg: '../img/sc-img/restaurants/goramen_05.jpg',
        eaten: [{
            menu: '카라이 돈코츠라멘 1 그릇 8000 원'
        }, {
            menu: '챠슈추가 1000 원'
        }],
        detail: '제 입맛엔 별로였어요. 너무 느끼하고 가게에서 발꼬락냄새 나요 ㅠㅠ' +
        '사장님도 겁나 쿨하심...',
        tag: [{
            item: "카라이돈코츠라멘"
        }, {
            item: "부타동"
        }, {
            item: "고라멘"
        }]
    }, {
        number: 6,
        userImg: '../img/sc-img/members/Jellyfish.jpg',
        userName: '알라코',
        likeCount: 2,
        trashCount: 0,
        achievement: [{
            emblem: '../img/sc-img/achievement/noodle.svg',
            achieve: '면요리 100회 리뷰'
        }, {
            emblem: '../img/sc-img/achievement/bbq.svg',
            achieve: '고기요리 100회 리뷰'
        }, {
            emblem: '../img/sc-img/achievement/pizza.svg',
            achieve: '피자 100회 리뷰'
        }],
        foodImg: '../img/sc-img/restaurants/goramen_01.jpg',
        eaten: [{
            menu: '돈코츠라멘 1 그릇 7000 원'
        }, {
            menu: '챠슈추가 1000 원'
        }],
        detail: '돼지뼈, 닭뼈, 생선 육수국물이<br>' +
        '아주 진해서 밥말아 먹기 딱 좋은 라멘!!',
        tag: [{
            item: "고라멘"
        }, {
            item: "돈코츠라멘"
        }, {
            item: "라멘"
        }, {
            item: "챠슈"
        }]
    }]
}];
